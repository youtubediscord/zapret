...
def normalize_winws2_action_lines(
    ...
    *,
    ...
    return normalize_out_range_action_lines(normalized)
    ...
    for line in action_lines:
        ...
        if inline_match:
            ...
            if parsed is not None and resolved_out_range is None:
                resolved_out_range = parsed
            normalized_body.append(cleanup_action_line_after_inline_out_range(stripped))
            fixes.append("lifted_inline_out_range")
            continue
...
def has_explicit_out_range(settings: OutRangeSettings | None) -> bool:
    ...
    return bool(
        ...
        and (mode in VALUELESS_OUT_RANGE_MODES or int(getattr(settings, "value", 0) or 0) > 0)
    )
