# profile/ui/profile_item.py
"""
Карточка профиля в списке пресета.

Новая раскладка намеренно разделена на две смысловые зоны:
- слева идентичность профиля: иконка, название, inline-метаинформация, badge;
...
class ProfileItem(CardWidget):
    """
    Карточка профиля в списке пресета.

    Строка состоит из двух зон:
    - слева смысловая часть профиля;
    - справа компактная колонка статуса и выбранной стратегии.
    ...
    def _build_ui(self):
        ...
        self._feedback_label = CaptionLabel("", right_widget)
        self._feedback_label.setProperty("tone", "muted")
        self._feedback_label.hide()
        right_layout.addWidget(self._feedback_label, 0, Qt.AlignmentFlag.AlignVCenter)
    ...
    def _apply_style(self):
        ...
        if self._feedback_label is not None:
            try:
                feedback_color = getattr(tokens, "fg_muted", "#b7bec8")
                if self._rating == "work":
                    feedback_color = "#49a35f"
                elif self._rating == "notwork":
                    feedback_color = "#d85c5c"
                elif self._favorite:
                    feedback_color = "#d9a441"
                self._feedback_label.setStyleSheet(
                    f"background: transparent; color: {feedback_color};"
                )
            except Exception:
                pass
    ...
    def set_feedback_state(self, rating: str = "", favorite: bool = False) -> None:
        self._rating = str(rating or "").strip().lower()
