from __future__ import annotations

from PyQt6.QtWidgets import QWidget

from winws_runtime.runtime.method_switch_flow import handle_launch_method_changed_runtime
from ui.navigation.sidebar_builder import sync_nav_visibility
from ui.navigation_pages import (
    resolve_preset_detail_back_page_for_method,
    resolve_preset_detail_root_page_for_method,
    resolve_profile_detail_root_page_for_method,
    resolve_zapret1_navigation_pages,
    resolve_zapret2_navigation_pages,
)
from ui.orchestra_runtime_actions import on_clear_learned_requested
from ui.page_contracts import PageSignalName, get_page_signal
from ui.page_names import PageName
from ui.profile_detail_workflow import (
    on_open_profile_detail,
    on_open_z1_profile_detail,
    on_profile_detail_selected,
    on_profile_detail_back,
    on_z1_profile_detail_selected,
)
from ui.strategy_selection_workflow import on_strategy_selected_from_page
from ui.workflows.common import get_current_launch_method
from ui.workflows.mode import (
    open_zapret2_preset_detail_for_method,
    open_zapret1_preset_detail,
    redirect_to_profiles_page_for_method,
    show_active_mode_control_page,
)
from ui.window_display_state import on_profile_ui_mode_changed

from .helpers import (
    connect_page_signal_if_present,
    connect_show_page_signal,
    connect_show_page_signal_if_present,
)


def _show_active_mode_control_page(window) -> None:
    show_active_mode_control_page(window, allow_internal=False)


def _open_zapret2_preset_detail(window, preset_name: str) -> None:
    open_zapret2_preset_detail_for_method(
        window,
        get_current_launch_method(),
        preset_name,
        allow_internal=True,
    )


def _open_zapret1_preset_detail(window, preset_name: str) -> None:
    open_zapret1_preset_detail(window, preset_name, allow_internal=True)


def _on_launch_method_changed(window, method: str) -> None:
    plan = handle_launch_method_changed_runtime(window, method)

    if plan.dispatch_action == "restart":
        log_text = (
            f"Метод '{method}' сохранён, дальнейший запуск передан в единый runtime pipeline"
        )
    elif plan.dispatch_action == "stop":
        log_text = (
            f"Метод '{method}' сохранён, активный DPI будет остановлен через единый runtime pipeline"
        )
    else:
        log_text = f"Метод '{method}' сохранён без немедленного запуска"

    try:
        from log.log import log


        log(log_text, "INFO")
    except Exception:
        pass

    try:
        sync_nav_visibility(window, method)
    except Exception:
        pass

    try:
        redirect_to_profiles_page_for_method(window, method)
    except Exception:
        pass


def connect_mode_page_signals(window, page_name: PageName, page: QWidget) -> None:
    z1_pages = resolve_zapret1_navigation_pages()
    z2_pages = resolve_zapret2_navigation_pages()

    if page_name == PageName.ZAPRET2_MODE:
        connect_page_signal_if_present(
            window,
            "z2_pages.open_profile_detail",
            page,
            PageSignalName.OPEN_PROFILE_DETAIL,
            lambda profile_key, current_strategy_id, w=window: on_open_profile_detail(w, profile_key, current_strategy_id),
        )

    if page_name in (z2_pages.profiles_page, z2_pages.user_presets_page, PageName.BLOBS):
        connect_page_signal_if_present(
            window,
            f"back_to_control.{page_name.name}",
            page,
            PageSignalName.BACK_CLICKED,
            lambda w=window: _show_active_mode_control_page(w),
        )

    if page_name == z2_pages.user_presets_page:
        connect_page_signal_if_present(
            window,
            f"{page_name.name}.preset_open_requested",
            page,
            PageSignalName.PRESET_OPEN_REQUESTED,
            lambda preset_name, w=window: _open_zapret2_preset_detail(w, preset_name),
        )

    if page_name == z2_pages.preset_detail_page:
        connect_show_page_signal_if_present(
            window,
            "z2_preset_detail.back_clicked",
            page,
            PageSignalName.BACK_CLICKED,
            resolve_preset_detail_back_page_for_method("zapret2_mode"),
            allow_internal=True,
        )
        connect_show_page_signal_if_present(
            window,
            "z2_preset_detail.navigate_to_root",
            page,
            PageSignalName.NAVIGATE_TO_ROOT,
            resolve_preset_detail_root_page_for_method("zapret2_mode"),
        )

    if page_name == z2_pages.control_page:
        signal_obj = get_page_signal(page, PageSignalName.NAVIGATE_TO_PRESETS)
        if signal_obj is not None:
            connect_show_page_signal(window, f"{page_name.name}.navigate_to_presets", signal_obj, z2_pages.user_presets_page, allow_internal=True)

        signal_obj = get_page_signal(page, PageSignalName.NAVIGATE_TO_PROFILES)
        if signal_obj is not None:
            connect_show_page_signal(window, f"{page_name.name}.navigate_to_profiles", signal_obj, z2_pages.profiles_page, allow_internal=True)

        signal_obj = get_page_signal(page, PageSignalName.NAVIGATE_TO_BLOBS)
        if signal_obj is not None:
            connect_show_page_signal(window, f"{page_name.name}.navigate_to_blobs", signal_obj, PageName.BLOBS, allow_internal=True)

        connect_page_signal_if_present(
            window,
            f"{page_name.name}.profile_ui_mode_changed",
            page,
            PageSignalName.PROFILE_UI_MODE_CHANGED,
            lambda mode, w=window: on_profile_ui_mode_changed(w, mode),
        )

    if page_name == z2_pages.profile_detail_page:
        connect_page_signal_if_present(window, "profile_detail.back_clicked", page, PageSignalName.BACK_CLICKED, lambda w=window: on_profile_detail_back(w))
        connect_show_page_signal_if_present(
            window,
            "profile_detail.navigate_to_root",
            page,
            PageSignalName.NAVIGATE_TO_ROOT,
            resolve_profile_detail_root_page_for_method("zapret2_mode"),
        )
        connect_page_signal_if_present(
            window,
            "profile_detail.strategy_selected",
            page,
            PageSignalName.STRATEGY_SELECTED,
            lambda profile_key, strategy_id, w=window: on_profile_detail_selected(w, profile_key, strategy_id),
        )
    if page_name in (z1_pages.profiles_page, z1_pages.user_presets_page):
        connect_show_page_signal_if_present(window, f"back_to_z1_control.{page_name.name}", page, PageSignalName.BACK_CLICKED, z1_pages.control_page)

    if page_name == z1_pages.user_presets_page:
        connect_page_signal_if_present(
            window,
            "z1_user_presets.preset_open_requested",
            page,
            PageSignalName.PRESET_OPEN_REQUESTED,
            lambda preset_name, w=window: _open_zapret1_preset_detail(w, preset_name),
        )

    if page_name == z1_pages.preset_detail_page:
        connect_show_page_signal_if_present(window, "z1_preset_detail.back_clicked", page, PageSignalName.BACK_CLICKED, resolve_preset_detail_back_page_for_method("zapret1_mode"), allow_internal=True)
        connect_show_page_signal_if_present(window, "z1_preset_detail.navigate_to_root", page, PageSignalName.NAVIGATE_TO_ROOT, resolve_preset_detail_root_page_for_method("zapret1_mode"))

    if page_name == z1_pages.profiles_page:
        connect_page_signal_if_present(
            window,
            "z1_pages.open_profile_detail",
            page,
            PageSignalName.OPEN_PROFILE_DETAIL,
            lambda profile_key, current_strategy_id, w=window: on_open_z1_profile_detail(w, profile_key, current_strategy_id),
        )

    if page_name == z1_pages.profile_detail_page:
        connect_show_page_signal_if_present(window, "z1_profile_detail.back_clicked", page, PageSignalName.BACK_CLICKED, z1_pages.profiles_page, allow_internal=True)
        connect_show_page_signal_if_present(window, "z1_profile_detail.navigate_to_root", page, PageSignalName.NAVIGATE_TO_ROOT, resolve_profile_detail_root_page_for_method("zapret1_mode"))
        connect_page_signal_if_present(
            window,
            "z1_profile_detail.strategy_selected",
            page,
            PageSignalName.STRATEGY_SELECTED,
            lambda profile_key, strategy_id, w=window: on_z1_profile_detail_selected(w, profile_key, strategy_id),
        )

    if page_name == z1_pages.control_page:
        connect_show_page_signal_if_present(window, "z1_control.navigate_to_profiles", page, PageSignalName.NAVIGATE_TO_PROFILES, z1_pages.profiles_page, allow_internal=True)
        connect_show_page_signal_if_present(window, "z1_control.navigate_to_presets", page, PageSignalName.NAVIGATE_TO_PRESETS, z1_pages.user_presets_page, allow_internal=True)
        connect_show_page_signal_if_present(window, "z1_control.navigate_to_blobs", page, PageSignalName.NAVIGATE_TO_BLOBS, PageName.BLOBS, allow_internal=True)

    if page_name == PageName.DPI_SETTINGS:
        connect_page_signal_if_present(window, "dpi_settings.launch_method_changed", page, PageSignalName.LAUNCH_METHOD_CHANGED, lambda method, w=window: _on_launch_method_changed(w, method))

    if page_name in (PageName.ZAPRET1_MODE, PageName.ZAPRET2_MODE):
        connect_page_signal_if_present(
            window,
            f"strategy_selected.{page_name.name}",
            page,
            PageSignalName.STRATEGY_SELECTED,
            lambda strategy_id, strategy_name, w=window: on_strategy_selected_from_page(w, strategy_id, strategy_name),
        )

    if page_name == PageName.ORCHESTRA:
        connect_page_signal_if_present(
            window,
            "orchestra.clear_learned_requested",
            page,
            PageSignalName.CLEAR_LEARNED_REQUESTED,
            lambda w=window: on_clear_learned_requested(w),
        )


__all__ = ["connect_mode_page_signals"]
