"""Item-level actions workflow для страницы пользовательских пресетов Zapret 1."""
...
def handle_item_dropped_action(*, source_kind: str, source_id: str, target_kind: str, target_id: str, storage_api, runtime_service, refresh_presets_view_from_cache_fn, log_fn) -> None:
    try:
...
def show_rating_menu_action(*, page, name: str, global_pos, resolve_display_name_fn, hierarchy_store, refresh_callback, tr_fn, show_preset_rating_menu_fn) -> None:
    display_name = resolve_display_name_fn(name)
...
def duplicate_preset_action(*, name: str, resolve_display_name_fn, actions_api, runtime_service, info_bar_cls, tr_fn, parent_window, log_fn) -> None:
    try:
        display_name = resolve_display_name_fn(name)
...
def reset_preset_action(*, name: str, resolve_display_name_fn, actions_api, message_box_cls, info_bar_cls, tr_fn, parent_window, log_fn) -> None:
    try:
        display_name = resolve_display_name_fn(name)
        if message_box_cls:
            box = message_box_cls(
...
def delete_preset_action(*, name: str, resolve_display_name_fn, storage_api, actions_api, runtime_service, message_box_cls, info_bar_cls, tr_fn, parent_window, log_fn) -> None:
    try:
        display_name = resolve_display_name_fn(name)
...
def export_preset_action(*, page, name: str, resolve_display_name_fn, file_dialog_cls, actions_api, info_bar_cls, tr_fn, parent_window, log_fn) -> None:
    display_name = resolve_display_name_fn(name)
