"""Lifecycle/theme/language helper'ы для страницы пользовательских пресетов Zapret 1."""
...
def schedule_user_presets_layout_resync(*, cleanup_in_progress: bool, layout_resync_timer, layout_resync_delayed_timer, include_delayed: bool = False) -> None:
    if cleanup_in_progress:
        return
...
def resync_user_presets_layout_metrics(*, cleanup_in_progress: bool, toolbar_layout, viewport, layout, update_presets_view_height_fn) -> None:
    if cleanup_in_progress:
        return
...
def apply_user_presets_page_theme(*, get_theme_tokens_fn, get_semantic_palette_fn, get_cached_qta_pixmap_fn, get_themed_qta_icon_fn, schedule_layout_resync_fn, configs_icon, reset_all_btn, presets_list, previous_theme_key, force: bool = False, log_fn=None):
    try:
...
def cleanup_user_presets_page(*, set_cleanup_in_progress_fn, layout_resync_timer, layout_resync_delayed_timer, preset_search_timer, current_unsubscribe, set_unsubscribe_fn, set_store_fn, stop_watching_presets_fn) -> None:
    set_cleanup_in_progress_fn(True)
    for timer in (layout_resync_timer, layout_resync_delayed_timer, preset_search_timer):
        try:
            timer.stop()
        except Exception:
            pass
