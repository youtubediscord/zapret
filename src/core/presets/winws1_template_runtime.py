...
def get_template_content_winws1(name: str) -> Optional[str]:
...
def get_template_canonical_name_winws1(name: str) -> Optional[str]:
    key = str(name or "").strip().lower()
    if not key:
        return None
...
def get_default_template_content_winws1() -> Optional[str]:
    canonical = get_template_canonical_name_winws1("Default")
    if canonical:
        return get_template_content_winws1(canonical)
...
def get_builtin_preset_content_winws1(name: str) -> Optional[str]:
    content = get_template_content_winws1(name)
    if content:
        return content
    if str(name or "").strip().lower() == "default":
        return _normalize_template_header_winws1(_DEFAULT_TEMPLATE_CONTENT, "Default")
    return None


def get_builtin_base_from_copy_name_winws1(name: str) -> Optional[str]:
    ...
    if not changed or not base:
        return None
    return get_template_canonical_name_winws1(base)


def ensure_winws1_templates_copied_to_presets() -> int:
    return 0


def update_changed_winws1_templates_in_presets() -> int:
    return 0


def reset_user_overrides_to_builtin_winws1() -> tuple[int, int, list[str]]:
    templates = _load_template_contents()
    if not templates:
        return (0, 0, [])

    presets_dir = _user_presets_dir_winws1()
    removed = 0
    failed: list[str] = []
    ...
    for name in sorted(templates.keys(), key=lambda value: value.lower()):
        ...
        try:
            ...
            failed.append(name)
            log(f"Failed to reset winws1 user override '{name}' to built-in: {exc}", "DEBUG")

    return (removed, len(templates), failed)


def overwrite_winws1_templates_to_presets() -> tuple[int, int, list[str]]:
    return reset_user_overrides_to_builtin_winws1()


def ensure_default_preset_exists_winws1() -> bool:
    try:
        presets_dir = _builtin_presets_dir_winws1()
        ...
        log(f"Error ensuring winws1 default preset: {exc}", "DEBUG")
        return False

def _user_presets_dir_winws1() -> Path:
    try:
...
def _builtin_presets_dir_winws1() -> Path:
    try:
...
def _load_template_contents() -> dict[str, str]:
    try:
        return load_repo_builtin_templates(
            _builtin_presets_dir_winws1(),
            normalize_content=_normalize_template_header_winws1,
        )
    except Exception as exc:
        log(f"Error reading winws1 built-in templates: {exc}", "DEBUG")
        return {}


def _normalize_template_header_winws1(content: str, preset_name: str) -> str:
