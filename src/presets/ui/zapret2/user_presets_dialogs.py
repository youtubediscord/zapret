...
class CreatePresetDialog(MessageBoxBase):
    ...
    def __init__(self, existing_names: list, parent=None, language: str = "ru"):
        ...
        self.titleLabel = SubtitleLabel(
            self._tr("page.winws2_user_presets.dialog.create.title", "Новый пресет"),
            self.widget,
        )
        ...
        name_label = BodyLabel(
            self._tr("page.winws2_user_presets.dialog.create.name", "Название"),
            self.widget,
        )
        ...
        source_label = BodyLabel(
            self._tr("page.winws2_user_presets.dialog.create.source", "Создать на основе"),
            self.widget,
        )
        ...
        try:
            ...
            self._source_seg.addItem(
                "standard",
                self._tr("page.winws2_user_presets.dialog.create.source.standard", "Встроенного пресета"),
            )
        ...
        self.yesButton.setText(self._tr("page.winws2_user_presets.dialog.create.button.create", "Создать"))
        self.cancelButton.setText(self._tr("page.winws2_user_presets.dialog.button.cancel", "Отмена"))
    ...
    def validate(self) -> bool:
        ...
        if not name:
            self.warningLabel.setText(
                self._tr("page.winws2_user_presets.dialog.validation.enter_name", "Введите название.")
            )
...
class RenamePresetDialog(MessageBoxBase):
    ...
    def __init__(self, current_name: str, existing_names: list, parent=None, language: str = "ru"):
        ...
        self.titleLabel = SubtitleLabel(
            self._tr("page.winws2_user_presets.dialog.rename.title", "Переименовать"),
            self.widget,
        )
        self.subtitleLabel = BodyLabel(
            self._tr(
        ...
        name_label = BodyLabel(
            self._tr("page.winws2_user_presets.dialog.rename.new_name", "Новое имя"),
            self.widget,
        )
        self.nameEdit = LineEdit(self.widget)
        self.nameEdit.setText(self._current_name)
        self.nameEdit.setPlaceholderText(
            self._tr("page.winws2_user_presets.dialog.rename.placeholder", "Новое имя...")
        )
        ...
        self.yesButton.setText(self._tr("page.winws2_user_presets.dialog.rename.button", "Переименовать"))
        self.cancelButton.setText(self._tr("page.winws2_user_presets.dialog.button.cancel", "Отмена"))
        self.widget.setMinimumWidth(420)
...
class ResetAllPresetsDialog(MessageBoxBase):
    ...
    def __init__(self, parent=None, language: str = "ru"):
        ...
        self.cancelButton.setText(
            tr_presets_dialog("page.winws2_user_presets.dialog.button.cancel", self._ui_language, "Отмена")
        )
