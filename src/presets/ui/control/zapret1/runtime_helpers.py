...
def apply_z1_pages_language(
    *,
    ...
    if open_profiles_btn is not None:
        open_profiles_btn.setText(tr_catalog("page.z1_control.button.open", language=language, default="Открыть"))
    ...
    if profiles_card is not None:
        profiles_card.setTitle(
            tr_catalog("page.z1_control.profiles.title", language=language, default="Профили и готовые стратегии")
        )
        profiles_card.setContent(
            tr_catalog("page.z1_control.profiles.desc", language=language, default="Выбор профиля и готовой стратегии из выбранного пресета")
        )
        profiles_card.button.setText(
