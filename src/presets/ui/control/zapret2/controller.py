...
class AdvancedSettingsLoadWorker(QThread):
    ...
    def __init__(self, request_id: int, app_context, parent=None):
...
class Zapret2ModeControlPageController(ControlPageController):
    @staticmethod
    ...
    @staticmethod
    def apply_profile_ui_mode_change(*, wanted_mode: str, app_context, reload_host) -> None:
        _ = (wanted_mode, app_context, reload_host)

        try:
            from settings.dpi.strategy_settings import set_profile_ui_mode

            set_profile_ui_mode(PROFILE_UI_MODE_DEFAULT)
        except Exception:
            pass
