...
def duplicate_preset_action(*, name: str, resolve_display_name_fn, actions_api, runtime_service, info_bar_cls, tr_fn, parent_window, log_fn, tr_prefix: str) -> None:
    try:
        display_name = resolve_display_name_fn(name)
...
def reset_preset_action(*, name: str, resolve_display_name_fn, actions_api, message_box_cls, info_bar_cls, tr_fn, parent_window, log_fn, tr_prefix: str) -> None:
    try:
        display_name = resolve_display_name_fn(name)
        if message_box_cls:
            box = message_box_cls(
                tr_fn(_tr_key(tr_prefix, "dialog.reset_single.title"), "Вернуть встроенный пресет?"),
                tr_fn(
                    _tr_key(tr_prefix, "dialog.reset_single.body"),
            ...
            box.yesButton.setText(tr_fn(_tr_key(tr_prefix, "dialog.reset_single.button"), "Вернуть встроенный"))
            box.cancelButton.setText(tr_fn(_tr_key(tr_prefix, "dialog.button.cancel"), "Отмена"))
            if not box.exec():
                return
...
def delete_preset_action(*, name: str, resolve_display_name_fn, storage_api, actions_api, runtime_service, message_box_cls, info_bar_cls, tr_fn, parent_window, log_fn, tr_prefix: str) -> None:
    try:
        display_name = resolve_display_name_fn(name)
        ...
        if message_box_cls:
            box = message_box_cls(
                tr_fn(_tr_key(tr_prefix, "dialog.delete_single.title"), "Удалить пресет?"),
                tr_fn(
                    _tr_key(tr_prefix, "dialog.delete_single.body"),
            ...
            box.yesButton.setText(tr_fn(_tr_key(tr_prefix, "dialog.delete_single.button"), "Удалить"))
            box.cancelButton.setText(tr_fn(_tr_key(tr_prefix, "dialog.button.cancel"), "Отмена"))
            if not box.exec():
                return
        result = actions_api.delete_preset(file_name=name, display_name=display_name)
...
def export_preset_action(*, page, name: str, resolve_display_name_fn, file_dialog_cls, actions_api, info_bar_cls, tr_fn, parent_window, log_fn, tr_prefix: str) -> None:
    display_name = resolve_display_name_fn(name)
    file_path, _ = file_dialog_cls.getSaveFileName(
        page,
        tr_fn(_tr_key(tr_prefix, "file_dialog.export_title"), "Экспортировать пресет"),
        f"{display_name}.txt",
    ...
    try:
        ...
        if result.infobar_level == "success":
            info_bar_cls.success(title=result.infobar_title, content=result.infobar_content, parent=parent_window)
    except Exception as exc:
