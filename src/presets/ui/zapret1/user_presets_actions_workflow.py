"""Actions workflow для страницы пользовательских пресетов Zapret 1."""
...
def import_preset_action(
    *,
    ...
    file_path, _ = file_dialog_cls.getOpenFileName(
        parent,
        tr_fn("page.z1_user_presets.file_dialog.import_title", "Импортировать пресет"),
        "",
    ...
    try:
        ...
        info_bar_cls.error(
            title=tr_fn("common.error.title", "Ошибка"),
            content=tr_fn("page.z1_user_presets.error.import_exception", "Ошибка импорта: {error}", error=exc),
            parent=parent_window,
        )
